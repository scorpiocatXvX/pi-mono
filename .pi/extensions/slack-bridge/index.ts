...
function quoteForSlack(text: string, maxChars = 500): string {
	const safe = text.trim();
	if (!safe) return "> _(empty message)_";
	const truncated = safe.length <= maxChars ? safe : `${safe.slice(0, maxChars)}...`;
	return truncated
		.split(/\r?\n/)
		.map((line) => `> ${line}`)
		.join("\n");
...
export default function (pi: ExtensionAPI) {
	...
	// Default to off to avoid double-consumption when external Slack service is running.
	...
	async function postReply(req: SlackRequest, replyText: string): Promise<void> {
		...
		const quoted = quoteForSlack(req.originalText || req.text);

		await webClient.chat.postMessage({
			channel: req.channel,
			text: truncateForSlack(`${quoted}\n\n${replyText}`),
		});
	}
	...
	async function connectSlack(_ctx: ExtensionContext): Promise<boolean> {
		...
		const handleIncomingMessage = async (envelope: SlackEventEnvelope): Promise<void> => {
			...
			const normalized = stripBotMention(originalText, botUserId);
